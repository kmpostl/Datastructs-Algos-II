...
if [ ! -f TopoPaths.java ]; then
	echo ""
	echo " Error: You must place TopoPaths.java in this directory before we"
...
for i in `seq 3 5`;
do
	if [ ! -f input_files/TestCase0$i-graph.txt ]; then
		echo ""
		echo " Error: You must place TestCase0$i-graph.txt in the input_files directory"
		echo "        before we can proceed. Aborting test script."
		echo ""
		exit
	fi
...
do
	...
	javac TopoPaths.java TestCase$i.java 2> /dev/null
	compile_val=$?
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " TopoPaths.java | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" TopoPaths.java | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" TopoPaths.java | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" TopoPaths.java -n"
		echo "     grep \"\$(printf '^\t')\" TopoPaths.java -n"
